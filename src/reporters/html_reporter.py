...
<body>
    <div class="container">
        ...
        <div class="footer">
            Generated by Vibe-Code Auditor v1.10.0 |
    ...
    def _render_issues(self, issues: list) -> str:
        """Render static analysis issues with priority sorting."""
        if not issues:
        ...
        for issue in issues:
            ...
            # 심각도가 정의된 범위 내에 있는지 확인
            if severity in issues_by_severity:
                issues_by_severity[severity].append(issue)
            else:
                # 알 수 없는 심각도는 info로 분류
                issues_by_severity['info'].append(issue)

        # 우선순위 순서: critical -> warning -> info
        ...
        for severity in priority_order:
            ...
            <div style="margin-bottom: 20px;">
                <h3 style="color: {'#dc3545' if severity == 'critical' else '#ffc107' if severity == 'warning' else '#28a745'}; 
                           margin-bottom: 10px; font-size: 1.2em;">
                    {severity_label} ({len(severity_issues)}개)
                </h3>
            """
    ...
    def _render_ai_issues(self, issues: list) -> str:
        """Render AI analysis issues with priority sorting."""
        if not issues:
    ...
    def _save_to_file(self, html_content: str, output_file: Path) -> None:
        ...
        try:
            ...
            logger.error("Failed to save HTML report: %s", e, exc_info=True)
            raise
